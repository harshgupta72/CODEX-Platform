...
export default function SignInPage() {
  ...
  // Ensure modal stays open until manually closed and blocks all navigation
  useEffect(() => {
    if (showErrorModal) {
      // Prevent any auto-close behavior and block all interactions
  ...
  const handleEmailSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (userDoc.exists()) {
        ...
        // User exists in Firebase Auth but not in our database - they need to sign up first
        await signOut(auth); // Sign them out since they're not registered
        setErrorMessage("You are not a registered user. First sign up to create your account.");
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (userDoc.exists()) {
        ...
        // User doesn't exist in our database - they need to sign up first
        await signOut(auth); // Sign them out since they're not registered
        setErrorMessage("You are not a registered user. First sign up to create your account.");
        setShowErrorModal(true);
      }
  ...
  const handleCloseModal = () => {
    ...
    setErrorMessage("");
    // After modal is closed, redirect to role selection page
    setTimeout(() => {
      router.push("/auth/role-selection");
    }, 300); // Small delay to allow modal to close animation
  };
